...
- 拆分 AGENTS.md 时，按"哪些内容会被同一类 agent 读取"分组，不要按原始文件中的章节顺序机械拆分。
- docs/ 每类文件（设计文档、规格、参考）使用独立的前言模板，帮助 agent 快速判断是否继续深入读取。
- 迁移后的旧 AGENTS.md 条目保留 1 轮协作周期的软链接注释（`# 原在 AGENTS.md，已移至 docs/xxx.md`），再彻底清除。
- 断链修复后验证一次 `find docs -name '*.md' -exec grep -l '\\](' {} \\;` 确认无残余断链。
...
- **输出不走磁盘**：doc-gardener 只以对话输出报告结论，不落盘。违反时撤回创建的临时文件。
