...
**场景**：环境中没有安装Playwright、Puppeteer等浏览器自动化工具
**处理**：报告能力缺口，不退回读代码猜测
**示例**：
```
能力缺口报告：
- 缺失工具：Playwright/Puppeteer
- 影响：无法进行UI验证
- 建议：安装浏览器自动化工具或使用其他验证方式
- 临时方案：使用静态分析或人工验证
```
...
**场景**：监控系统不可用或查询语法错误
**处理**：验证监控系统连接状态，检查查询语法，必要时使用备用监控工具
**示例**：
```
监控系统检查：
1. 检查监控系统连接状态
2. 验证查询语法是否正确
3. 检查权限配置
4. 使用备用监控工具（如Prometheus、Grafana等）
```

### 边界情况3：历史数据污染

**场景**：历史日志/指标干扰当前验证
**处理**：验证前清理历史数据，或使用时间范围过滤
**示例**：
```
数据清理方案：
1. 使用时间范围过滤：`timestamp >= '2026-07-02T00:00:00Z'`
2. 清理历史数据：删除验证时间范围外的数据
3. 使用唯一标识：添加请求ID、会话ID等唯一标识
4. 隔离验证环境：使用独立的测试环境
```
...
```
跨服务验证方案：
1. 使用分布式追踪（如Jaeger、Zipkin）
2. 记录请求链路：服务A → 服务B → 服务C
3. 验证每个服务的性能指标
4. 检查链路中的错误和延迟
```
...
**场景**：需要验证移动端页面的显示和交互
**处理**：使用移动端浏览器自动化工具或真机测试
**示例**：
```
移动端验证方案：
1. 使用Playwright的移动端模拟
2. 使用真机测试（如BrowserStack、Sauce Labs）
3. 验证响应式设计
4. 测试触摸交互
```
