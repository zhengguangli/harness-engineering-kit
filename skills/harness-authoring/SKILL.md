...
跨平台工具映射表与 agent 双版本生成规则,详见 `references/skill-scaffolder-prompt.md` 中的跨平台注意事项。


### 跨平台 system_prompt 同步纪律

每个 agent 的 `.md`（Claude Code）和 `openai.yaml`（Codex）两个版本的 system_prompt **必须逐字一致**——仅允许工具名差异（`Bash` ↔ `exec_command`、`Edit` ↔ `apply_patch` 等，映射表见 `references/agent-template-codex.yaml` 注释）。不得出现一处版本比另一处更详细、更简略、或措辞微妙不同的情况——这类渐进式漂移是跨平台不一致的主要来源。
