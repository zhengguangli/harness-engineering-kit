...
**场景**：项目只有几个文件，没有重复模式
**处理**：不需要建立周期性扫描机制，使用简单的代码review即可
**示例**：
```
项目规模判断：
- 文件数量 < 10
...
**处理**：为每种语言建立独立的规则，使用统一的扫描工具
**示例**：
```
多语言规则管理：
1. 为每种语言建立独立的lint规则
2. 使用统一的扫描工具（如ESLint、Pylint、Checkstyle）
3. 建立语言特定的最佳实践
4. 定期审计规则的有效性
```
...
**场景**：多个团队协作开发，需要统一代码风格
**处理**：建立统一的代码风格指南，各团队在指南内自由实现
**示例**：
```
团队协作方案：
1. 建立统一的代码风格指南
