...
- 用户进入一个新项目目录，第一次对话时说 hello 或简单问候（此时不要自动执行分析，应先回复并询问“需要我分析一下这个项目吗？”；得到明确确认后再执行卡片生成）。
...
| 2 | `cat package.json` / `Cargo.toml` / `go.mod` / `pyproject.toml`（若根目录无包管理文件，扫描 depth-2 子目录查找） | 语言、框架、运行时、依赖 |
