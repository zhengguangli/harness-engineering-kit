...
- 按成本递增采集信息，每一步都可能已足够产出卡片，避免过度探索。
- 信息缺失时写"未发现"或"未配置"，绝不猜测。
- 5 维度信息在 6-8 个工具调用内完成采集。
...
1. 读取 `README.md` → 项目名称、一句话描述、关键约束、构建命令（不输出全文）
2. 读取包管理文件 → 技术栈（语言、框架、运行时、包管理器、部署目标）；全部缺失时 `ls` 推断并标注
3. `Glob` / `find . -maxdepth 2` → 目录骨架与模块划分
4. `Grep` 搜索入口文件（main/index/app）→ 关键模块识别
5. `package.json` scripts / `Makefile` / `Justfile` → 构建、测试、运行命令
6. `git log --oneline -10` → 近期 commit、版本号、贡献者
7. 按项目卡片模板输出结构化卡片
8. 发现 README 过时、配置缺失时在"已知约束"中注明

### 约束

- **只读不改**：禁止文件写入、删除、修改；禁止 `npm install` 等改变文件系统的命令
- **不编造**：信息缺失时写"未发现"或"未配置"，不猜测
- **静默采集**：所有采集过程对用户不可见，只输出最终卡片
- **快速收敛**：5 维度在 6-8 个工具调用内完成采集
