...
- **Scope boundary**: The assessor writes only its own report (`docs/quality-reports/skills-quality-assessment.md`) and the score table (`docs/QUALITY_SCORE.md`). It must not modify any file under `skills/` — evaluation and repair are separate roles. On violation: revert the skill edit and hand the finding to `harness-authoring`.
