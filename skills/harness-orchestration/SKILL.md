...
### 4. Routing Quality Validation

Before presenting routing recommendations to the user, validate against these checks:

| Check | Pass condition | Fix if failed |
|-------|---------------|---------------|
| Workflow match | The recommended workflow directly addresses the user's stated goal | Re-run Layer 1-3 matching |
| No missing prerequisites | All upstream skills in the recommended sequence have their inputs available | Insert the missing prerequisite skill |
| Omission justified | Every omitted skill has a documented reason (from the Omission Decision Guide) | Add the reason or un-omit the skill |
| Handoff points explicit | When spanning workflows, each handoff has named upstream outputs and downstream inputs | Document the handoff table |
| No over-engineering | The recommended sequence uses ≤ 5 skills for a single-goal task | Remove non-essential skills |

If any check fails, fix before presenting the recommendation.
...
## Hard Constraints
1. **Workflow 1 must not skip `project-intake`**: Violating this means the skeleton generated by `bootstrap` may not match the actual project, leading to rework. Violation → halt the bootstrap step, run project-intake first, then resume.
2. **Do not force orchestration on users who already know which skill to use**: When a user explicitly says '用 X skill', execute it directly. Violation → withdraw the routing output and invoke the user's chosen skill immediately.
3. **Routing recommendations must be specific and actionable**: Every recommendation must name the skill, the execution order, and the reason. Violation → supplement with concrete skill names and ordering before presenting to the user.
4. **Cross-workflow handoff prerequisites must be verified**: Before starting a downstream workflow, confirm upstream deliverables exist (e.g., project-intake output before bootstrap). Violation → pause the downstream workflow, produce the missing upstream deliverable, then resume.
...
- **After a new skill is added, refresh the routing table before trusting it.** A new skill is invisible to routing until four places agree: (1) the skill's own `SKILL.md` frontmatter declares `when_to_use` and a `depends_on` list; (2) `scripts/validate_skill_dependencies.py`'s `LAYERS` / `META_LAYER` constants name it, or the dependency check fails; (3) `docs/ARCHITECTURE.md`'s layering section lists it in the right layer; (4) `scripts/run_trigger_regression.py`'s `SKILL_KW` maps its trigger keywords, and `tests/triggers/cases.json` gains at least one case. Until all four are done, route around the new skill rather than recommending it — a route that lands on a skill the platform cannot resolve is worse than no route.
...
---
Last updated: 2026-09-24 (Change: added a Best Practice covering the four places that must be updated when a new skill is added; round-33 LOW #7)
## Agent 提示词
...
1. **Understand the goal**: Determine which workflow the user's intent belongs to, analyze the user's needs, project status, and technical context. If the goal is ambiguous, ask 1-2 targeted clarifying questions before routing — do not guess. **De-duplication check**: if the same goal was already routed in this session (same user intent + same workflow match), skip to step 3 and output "Continuing from previous routing — no changes needed" rather than re-running the full framework.
