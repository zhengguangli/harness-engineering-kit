...
## Hard Constraints
- **Workflow 1 must not skip `project-intake`**: Violating this means the skeleton generated by `bootstrap` may not match the actual project, leading to rework. Violation → halt the bootstrap step, run project-intake first, then resume.
- **Do not force orchestration on users who already know which skill to use**: When a user explicitly says '用 X skill', execute it directly. Violation → withdraw the routing output and invoke the user's chosen skill immediately.
- **Routing recommendations must be specific and actionable**: Every recommendation must name the skill, the execution order, and the reason. Violation → supplement with concrete skill names and ordering before presenting to the user.
- **Cross-workflow handoff prerequisites must be verified**: Before starting a downstream workflow, confirm upstream deliverables exist (e.g., project-intake output before bootstrap). Violation → pause the downstream workflow, produce the missing upstream deliverable, then resume.
...
---
Last updated: 2026-07-10 (Change: Hard Constraints expanded to 4 with violation consequences, Routing Quality Validation subsection, automation checks enhanced)
