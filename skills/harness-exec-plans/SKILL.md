...
description: 把跨多个上下文窗口/多次会话的复杂任务,落盘为版本化的"执行计划"(exec-plan)工件——包含目标、步骤、决策记录、验收标准与进度状态,而不是只在对话里口头规划。当任务复杂度足以让 agent 在中途"失忆"、需要长时程自主执行、或用户说"先做个计划再动手""这个改动比较大"时主动使用此技能。小的、一次性能做完的改动不需要 exec-plan,用临时的轻量计划即可。
---
