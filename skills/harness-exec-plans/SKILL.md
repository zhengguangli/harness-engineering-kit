...
创建或提交 exec-plan 前，逐项确认：

1. **一句话目标**：读完后是否能不翻上下文就知道"完成后世界变成什么样"？如果一句话说不清，目标太模糊。
2. **非目标是否覆盖**：不要只写做什么，还要写不做什么——防止执行中 scope creep。
3. **每步可独立验证**：每个 `- [ ]` 步骤完成后能否用一条命令或一次截图验证通过？不能则拆细。
4. **验收标准可机械检查**：标准是否能用脚本/命令/截图判断？"看起来不错"不算。
5. **决策日志是否非空**：已经做出但未记录的关键决策（"为什么选 A 不选 B"）要补充。执行中随记。
6. **风险是否已识别**：已知未知（技术探索型步骤）和外部依赖要明确标注。

如果第 1 项不通过，返回用户确认目标后再继续。其余项目任意不通过则修复后提交。
### 并行协作约定
...
- 创建 exec-plan 后立即标注首次预计用时，便于排障时判断是"正常慢"还是"卡住了"。
- 决策日志使用 "选 A 因为 X，放弃 B 因为 Y" 的对比格式，不要只写"选了 A"。
- 多 agent 协作时，每个 agent 完成步骤后在文件顶部更新负责人标记，避免编辑冲突。
- 搁置任务的 `tech-debt-tracker.md` 条目附上上下文窗口摘要路径（如对话历史文件），方便恢复时快速获取背景。
...
- **单路径落盘**：exec-plan 文件统一放在 `docs/exec-plans/active/` 目录，完成后再移入 `completed/`。违反时回撤到正确路径。
...
- 决策日志只记录有选择的地方。
- 不预写代码，不在计划里写实现细节。
- **落盘路径**：`docs/exec-plans/active/<plan-id>.md`（kebab-case 命名）。
