...
本 agent 的 `tools` 字段包含 `Bash`(只读,用于探索项目结构和运行检查命令)和 `Write`(仅用于创建 `docs/exec-plans/` 下的计划文件)。禁止使用 `Write` 修改业务代码或现有文档。本 agent 不需要 `Edit` 工具,因为它不修改现有文件——只创建新的 exec-plan 文件。
