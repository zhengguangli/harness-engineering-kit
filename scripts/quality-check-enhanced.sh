...
SKILLS_DIR="./skills"
REPORT_FILE="docs/quality-reports/quality-check-enhanced.md"

# 创建报告目录
mkdir -p docs/quality-reports

# 开始报告
echo "# 技能质量自动化检查报告（增强版）" > "$REPORT_FILE"
echo "" >> "$REPORT_FILE"
...
for skill in "${SKILLS[@]}"; do
    ...
    if [ -f "$SKILL_FILE" ]; then
        ...
        # 检查新章节
        if grep -q "^## 自动化友好度" "$SKILL_FILE"; then
            echo "- [x] 自动化友好度章节存在" >> "$REPORT_FILE"
        else
            echo "- [ ] 自动化友好度章节缺失" >> "$REPORT_FILE"
        fi
        
        if grep -q "^## 用户体验" "$SKILL_FILE"; then
            echo "- [x] 用户体验章节存在" >> "$REPORT_FILE"
        else
            echo "- [ ] 用户体验章节缺失" >> "$REPORT_FILE"
        fi
...
echo "增强版自动化检查完成，报告已保存到 $REPORT_FILE"